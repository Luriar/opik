"""
Run Delta Lake backfill on EC2 via SSM — using Java 17 (default)
"""
import boto3, time
...
def run_ssm(commands, timeout=300):
    ...
    while waited < timeout + 30:
        ...
        if elapsed > 30 and elapsed % 30 < 5:
        ...
        if s in ("Success", "Failed", "TimedOut", "Cancelled"):
            ...
            print(f"    STATUS={s} (elapsed={int(time.time()-start)}s)")
...
print(f"Command: {BACKFILL_CMD[:120]}...")
print(f"Using default Java 17")
