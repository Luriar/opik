...
# 기준: 매일 20:00 KST(밤 8시). incremental 수집창(평일 07:20~20:10) 끝물에 돌아 그날 적재분을 병합한다.
# 동시성: max_active_runs=1. 병합 시작 시점에 list된 part만 제거하고, 이후 증분 part는 보존한다(20:00~20:10
#         겹침에 안전: 그 시점 이후 쓰인 part는 list에 없어 다음 실행이 흡수).
with DAG(
    dag_id="dag_dart_gold_compaction",
    start_date=pendulum.datetime(2026, 1, 1, tz="Asia/Seoul"),
    schedule="0 20 * * *",  # 20:00 KST 매일 (밤 8시)
    catchup=False,
    max_active_runs=1,
