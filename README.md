...
| `opik_silver_extract` | `dags/silver/extract_silver.py` | 세 Bronze 완료 후 PDF를 S3 `silver/` JSON으로 변환 | 매일 00:00 KST, sensor 대기 |
...
- target date는 `data_interval_end`를 `Asia/Seoul`로 변환한 날짜입니다. 예약 실행과 수동 실행 모두 실행일의 한국 날짜를 사용합니다.
...
기존 `{{ ds }}`는 Airflow logical date의 UTC 날짜였습니다. 자정 이후 예약 실행은 이전 data interval의 시작 시각을 logical date로 사용하므로 6월 18일 새벽 실행이 6월 16일로 렌더링될 수 있었고, 오후 수동 실행은 현재 날짜를 사용해 결과가 달라졌습니다. 현재는 KST `data_interval_end`를 사용해 이 차이를 제거했습니다.
