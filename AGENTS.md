...
`AGENT.md` and `CODEX.md` are Codex-only runtime instruction files for this project. Because this repository is edited through both Codex and Claude, Codex must also read `AGENT_SHARE.md` for shared work heritage and `SHARE.md` for shared project knowledge before making project-level changes.

Do not duplicate long cross-agent project rules in this file. Put shared customization history and operating notes in `AGENT_SHARE.md`, and keep project design/operations details in `SHARE.md` or the linked `docs/` source-of-truth documents.
