        ...
        if _ddf is not None and len(_ddf) > 0 and "발행일" in _ddf.columns:
            ...
            for _, row in d.iterrows():
                ...
                results.append({
                    ...
                    "종목명": str(row.get("종목명", "")) if row.get("종목명") is not None else "",
                    "발행일": str(row.get("발행일", "")) if row.get("발행일") is not None else "",
                    "증권사": sec,
                    "reason": str(reason)[:300] if reason else None,
                    "투자의견": str(row.get("투자의견", "")) if row.get("투자의견") is not None and str(row.get("투자의견")) not in ("nan", "None") else "",
                    "목표주가": str(row.get("목표주가", "")) if row.get("목표주가") is not None and str(row.get("목표주가")) not in ("nan", "None") else "",
                    "상승여력_pct": str(row.get("상승여력_pct", "")) if row.get("상승여력_pct") is not None and str(row.get("상승여력_pct")) not in ("nan", "None") else "",
                    "keywords": info.get("keywords"),
                    "risks": info.get("risks"),
