...
COMPARE_STOCKS_PROMPT = """당신은 OPIK 금융 분석가입니다. 여러 종목의 증권사 리포트를 종목 간 비교 분석하세요.

## 출력 형식
```
### {종목A} vs {종목B} 증권사 애널리스트 비교
...
## 종합
{두 종목 간 애널리스트 시각 차이, 공통점, 상대적 포지셔닝을 3-4문장으로}

## 데이터 한계
- 제공된 리포트는 FAISS 검색 결과로 전체 coverage가 아닙니다.
- N/A로 표시된 필드는 Gold 추출 단계에서 값이 없었음을 의미합니다.
```

## 중요 규칙
- 각 종목을 동등하게 다루고, 어느 한쪽을 추천하는 뉘앙스 금지
- "A가 B보다 낫다"는 식의 표현 대신 "A는 X 측면에서, B는 Y 측면에서 강점을 보입니다" 형태로 작성
- 확정적 진술 금지
- 투자 판단 유도 금지
- 응답 마지막에 "※ 본 분석은 증권사 리포트의 비교 요약이며 투자 권유가 아닙니다." 추가"""
...
class AnalysisAgent:
    ...
    def compare_stocks(
        self, reports: List[dict], companies: List[str],
        stock_codes: List[str] = None
    ) -> str:
        """Compare analyst reports across multiple stocks (cross-stock comparison)."""
        if len(reports) < 2:
            return "비교할 리포트가 2개 이상 필요합니다."
        ...
        for r in reports:
            code = r.get("종목코드", "UNKNOWN")
            groups[code].append(r)

        # v6: Filter to only the tickers the user actually asked about
        if stock_codes:
            before_count = len(groups)
            groups = {k: v for k, v in groups.items() if k in stock_codes}
            logger.info(
                "compare_stocks: filtered %d tickers to %d requested: %s",
                before_count, len(groups), ", ".join(groups.keys()),
            )

        unique_codes = list(groups.keys())
        if len(unique_codes) < 2:
            logger.info(
                "compare_stocks: only 1 ticker (%s), fallback",
                unique_codes[0] if unique_codes else "?",
            )
            return self.compare_reports(reports, companies[0] if companies else "")

        logger.info(
            "compare_stocks: %d tickers (%s), %d reports",
            len(unique_codes), ", ".join(unique_codes), len(reports),
        )

        context_parts = []
        for code in unique_codes[:3]:
            stock_reports = groups[code]
            opinions = [r.get("투자의견", "N/A") for r in stock_reports]
            tp_values = [
                r.get("목표주가", "N/A")
                for r in stock_reports
                if r.get("목표주가") and r["목표주가"] != "N/A"
            ]

            context_parts.append(f"## {code} 증권사 리포트")
            context_parts.append(f"- 리포트 수: {len(stock_reports)}건")
            context_parts.append(f"- 투자의견 분포: {', '.join(opinions[:10])}")
            if tp_values:
                context_parts.append(f"- 목표주가 범위: {min(tp_values)}원 ~ {max(tp_values)}원")
        ...
        context = "\n".join(context_parts)
        return self._call_opus(COMPARE_STOCKS_PROMPT, context, max_tokens=2500)
